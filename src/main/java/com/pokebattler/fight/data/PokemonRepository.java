...
public interface PokemonRepository {
    public static final EnumSet<PokemonId> END_GAME_POKEMONS = EnumSet.copyOf(Arrays.asList(VENUSAUR, CHARIZARD, BLASTOISE, BUTTERFREE,
            ...
            LEDIAN, ARIADOS, CROBAT, LANTURN, TOGETIC, XATU, AMPHAROS, BELLOSSOM, AZUMARILL, SUDOWOODO, POLITOED, JUMPLUFF,
            ...
            DONPHAN, PORYGON2, STANTLER, TYROGUE, HITMONTOP, MILTANK, TYRANITAR));
    //ARTICUNO, MOLTRES, ZAPDOS, MEW, MEWTWO, RAIKOU, ENTEI, SUICUNE, LUGIA, HO-OH, CELEBI, SMEARGLE
    // top 30 CP
    public static final EnumSet<PokemonId> END_GAME_DEFENDER_POKEMONS =  EnumSet.copyOf(Arrays.asList(TYRANITAR, DRAGONITE,
    		SNORLAX, RHYDON, GYARADOS, BLISSEY, VAPOREON, DONPHAN, ESPEON, HERACROSS, GOLEM, EXEGGUTOR, FLAREON, MACHAMP, ALAKAZAM,
    		ARCANINE, SCIZOR, PINSIR, URSARING, JOLTEON, FERALIGATR, MUK, AMPHAROS, KINGLER, CHARIZARD, TYPHLOSION, OMASTAR, GENGAR, 
    		AERODACTYL, LAPRAS));
	...
	default Pokemons getAllEndGameDefender() {
        Pokemons.Builder builder = getAll().toBuilder().clearPokemon();
        getAll().getPokemonList().stream().filter(pokemon -> END_GAME_DEFENDER_POKEMONS.contains(pokemon.getPokemonId()))
        .forEach(pokemon -> builder.addPokemon(pokemon));
        return builder.build();
    }
