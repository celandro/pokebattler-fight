...
public class ResistRepository {
    ...
    public ResistRepository() {
        try (InputStream is =  this.getClass().getResourceAsStream("resists.csv");Scanner lineScanner = new Scanner(is)) {
            Map<PokemonType,Map<PokemonType,Float>> tempMap = new EnumMap<>(PokemonType.class);
            // skip line 1
            lineScanner.next();
            for (int i=0; lineScanner.hasNext();i++) {
                String s = lineScanner.next();
                try (Scanner r = new Scanner(s)) {
                    Scanner rowScanner = r.useDelimiter(",");
                    PokemonType attacker = PokemonType.forNumber(i);
                    Map<PokemonType,Float> defenseMap = new EnumMap<>(PokemonType.class);
                    // Ignore Unrecognized
                    // skip column 1
                    rowScanner.next();
                    for (int j=0; j< PokemonType.values().length - 1; j++) {
                        PokemonType defender = PokemonType.forNumber(j);
