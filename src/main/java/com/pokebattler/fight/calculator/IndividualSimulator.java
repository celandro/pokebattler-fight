...
public class IndividualSimulator implements AttackSimulator {
    ...
    /* (non-Javadoc)
	 * @see com.pokebattler.fight.calculator.Simulator#fight(com.pokebattler.fight.data.proto.FightOuterClass.Fight, boolean)
	 */
    @Override
	public FightResult.Builder fight(Fight fight, boolean includeDetails) {
        ...
        log.debug("{}: {} chose {} with {} energy", currentTime - Formulas.START_COMBAT_TIME, defenderState.getPokemonId(), defenderState.getNextMove().getMoveId(), defenderState.getCurrentEnergy());
        ...
        while (attackerState.isAlive() && defenderState.isAlive() && currentTime < Formulas.MAX_COMBAT_TIME_MS) {
            ...
            if (defenderState.getNextMove() == null) {
                ...
                log.debug("D{}: {} chose {}, gaining {} energy, new energy {}", currentTime - Formulas.START_COMBAT_TIME, 
                		defenderState.getPokemonId(), defenderState.getNextMove().getMoveId(), energyGain, defenderState.getCurrentEnergy());
            }
            if (attackerState.getNextMove() == null) {
                ...
                log.debug("A{}: {} chose {}  gaining {} energy, new energy {}", currentTime - Formulas.START_COMBAT_TIME, 
                		attackerState.getPokemonId(), attackerState.getNextMove().getMoveId(), energyGain, attackerState.getCurrentEnergy());
            }
            ...
            // tie goes to attacker
                ...
                log.debug("A{}: {} took {} damage and gained {} energy", currentTime - Formulas.START_COMBAT_TIME, defenderState.getPokemonId(), 
                ...
                log.debug("D{}: {} took {} damage and gained {} energy", currentTime - Formulas.START_COMBAT_TIME, attackerState.getPokemonId(), 
                        result.getDamage(), energyGain);
                ...
                log.debug("A{}: {} finished his attack", currentTime - Formulas.START_COMBAT_TIME, attackerState.getPokemonId());
                defenderState.moveTime(timeToNextAttack);
            } else {
                currentTime += timeToNextDefense;
                defenderState.resetAttack(timeToNextDefense);
                log.debug("D{}: {} finished his attack", currentTime - Formulas.START_COMBAT_TIME, defenderState.getPokemonId());
