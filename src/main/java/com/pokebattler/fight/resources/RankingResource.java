...
public class RankingResource {
    ...
    public Response rankAttacker(@PathParam("attackerLevel") String attackerLevel,
        ...
        log.debug("Calculating attacker rankings for attackerLevel {}, defenderLevel {}, attackStrategy {}, defenseStrategy {}", attackerLevel, defenderLevel, attackStrategy,
                defenseStrategy);
        ...
        return Response.ok(simulator.rankAttacker(attackerLevel, defenderLevel, attackStrategy, defenseStrategy)).cacheControl(cacheControl).build();
    ...
    public Response rankDefender(@PathParam("attackerLevel") String attackerLevel,
            ...
            @PathParam("attackStrategy") AttackStrategyType attackStrategy,
            @PathParam("defenseStrategy") AttackStrategyType defenseStrategy) {
        log.info("Calculating defender rankings for attackerLevel {}, defenderLevel {}, attackStrategy {}, defenseStrategy {}", attackerLevel, defenderLevel, attackStrategy,
                defenseStrategy);
        // set caching based on wether the result is random
        // TODO: refactor this to strategy pattern or change to a parameter?
        // maybe a query parameter to seed the rng?
        final javax.ws.rs.core.CacheControl cacheControl = new javax.ws.rs.core.CacheControl();
        cacheControl.setMaxAge(isRandom(attackStrategy, defenseStrategy) ? 0 : 86000);
        return Response.ok(simulator.rankDefender(attackerLevel, defenderLevel, attackStrategy, defenseStrategy)).cacheControl(cacheControl).build();
