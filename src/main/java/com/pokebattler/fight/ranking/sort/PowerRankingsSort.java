...
public class PowerRankingsSort implements RankingsSort {
	...
	public PowerRankingsSort() {
		
	}
	public PowerRankingsSort(CachingRankingSimulator cachingSimulator, PokemonDataCreator creator) {
	...
	public Comparator<SubResultTotalOrBuilder> getSubResultComparator() {
		...
		return Comparator.<SubResultTotalOrBuilder>comparingDouble(total -> -total.getPower()).thenComparing(
				Comparator.<SubResultTotalOrBuilder>comparingInt(total -> -total.getNumWins()).thenComparing(
						Comparator.<SubResultTotalOrBuilder>comparingInt(total -> -total.getDamageDealt())));
	}

	@Override
	public Comparator<FightResultOrBuilder> getFightResultComparator() {
		// for the defender section, try and find the worst case not the best
		// case!
		return Comparator.<FightResultOrBuilder>comparingDouble(result -> -result.getPower())
				.thenComparing(Comparator.comparingInt(result -> -(result.getWin() ? 1 : 0)))
				.thenComparing(Comparator.comparingInt(result -> -result.getCombatantsOrBuilder(0).getDamageDealt()));
	}
    ...
    public RankingsSort getRelativeSort(RankingParams params) {
		...
		rankingResult.getAttackersList().forEach(attacker -> {
			attacker.getByMoveList().forEach(moveset -> {
				...
				relative.put(d, 1.0/moveset.getTotal().getPower());
			});
		});
    	return new PowerRankingsSort(this.cachingSimulator, this.creator) {
    	    ...
    	    public Comparator<DefenderResultOrBuilder> getDefenderResultComparator() {
    	        return Comparator.<DefenderResultOrBuilder>comparingDouble(result->{
    	        	try {
	    	        	...
	    	        	double retval = result.getTotalOrBuilder().getPower() / bestOverall;
	    	        	return retval;
