...
//                dodgedSpecial = false;
//                timeElapsed += move1.getDurationMs() + extraDelay;
//                return new PokemonAttack(move1.getMoveId(), extraDelay);
//              }else{
...
//              }
  ...
  private int calculateEarliestNextDamageTime(CombatantState attackerState, CombatantState defenderState){
    ...
    if(defenderState.getNumAttacks() < 3){
      ...
      // Im not 100% confident on this method of randomly missing. I feel that moving the randomization of dodging to
      // an injected helper would be much better
//      if(isRandomStrategy()){
