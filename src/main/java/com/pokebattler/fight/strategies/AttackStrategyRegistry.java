...
//        register(strategy16);
    ...
    public AttackStrategy create(AttackStrategyType name, PokemonData pokemon, DodgeStrategy dodgeStrategy) {
        if (!strategies.containsKey(name)) {
            throw new IllegalArgumentException("Unimplemented attack strategy " + name);
        }
        return strategies.get(name).build(pokemon, dodgeStrategy);
    }
